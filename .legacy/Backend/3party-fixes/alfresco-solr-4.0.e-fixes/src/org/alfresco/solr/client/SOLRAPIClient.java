...
public class SOLRAPIClient
{
    ...
    {
    	log.info("using Edu-Sharing fixed Version for alfresco 4.0.e !");
    	System.out.println("org.alfresco.solr.client.SOLRAPIClient: using Edu-Sharing fixed Version for alfresco 4.0.e !");
    ...
    private PropertyValue getSinglePropertyValue(DataTypeDefinition dataType, Object value) throws JSONException
    {
    	...
    	log.info("!!! dataType:"+dataType.getName() + " value:"+value);
        ...
        else if(dataTypeName.equals(DataTypeDefinition.MLTEXT) && (value instanceof JSONArray))
        {
    ...
    private PropertyValue getPropertyValue(PropertyDefinition propertyDef, Object value) throws JSONException
    {
        ...
        {
        	log.info("propertyDef: "+propertyDef.getName());
            ...
            if(isMulti)
            {
                ...
                {
                	log.info("value class:"+array.get(j).getClass().getName() +" V:"+array.get(j));
                    if(propertyDef.getDataType().getName().equals(DataTypeDefinition.MLTEXT)){
                    	
                    	multi.addValue(getSinglePropertyValue(dataType, new JSONArray((String)array.get(j))));
                    }else{
    ...
    public List<NodeMetaData> getNodesMetaData(NodeMetaDataParameters params, int maxResults) throws AuthenticationException, IOException, JSONException
    {
        ...
        body.put("maxResults", maxResults);
        //edu-sharing debug cause of indexin problems after importing dmg objects
        log.info("es: params.getFromNodeId():"+params.getFromNodeId()+" params.getToNodeId():"+params.getToNodeId()+" FromTxnId:" + params.getFromTxnId() + " ToTxnId:" + params.getToTxnId());
        PostRequest req = new PostRequest(url.toString(), body.toString(), "application/json");
        Response response = null;
        JSONObject json = null;
        try
