...
install() {
  ...
  pushd $ALF_HOME >/dev/null || exit
  mvn dependency:copy -Dartifact=org.edu_sharing:edu_sharing-community-deploy-installer-repository:maven-develop-SNAPSHOT:tar.gz:bin #>/dev/null || exit
  #PROJECT_VERSION=$(mvn -q help:evaluate -Dexpression=project.version -DforceStdout)
  tar -xzvf ${BUILD_PATH}/target/edu_sharing-community-deploy-installer-repository-${PROJECT_VERSION}-bin.tar.gz -C $ALF_HOME  >/dev/null || exit
  popd >/dev/null || exit
...
*)
  ...
  echo "  - install <alfresco directory>:            start the installation"
  echo ""
  ;;
esac
