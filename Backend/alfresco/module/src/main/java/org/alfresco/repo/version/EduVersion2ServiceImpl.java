...
public class EduVersion2ServiceImpl extends org.alfresco.repo.version.Version2ServiceImpl {
    ...
    private static final Collection<String> typesToKeep = Arrays.asList(
    ...
    public void revert(NodeRef nodeRef, Version version, boolean deep) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Run as user " + AuthenticationUtil.getRunAsUser());
            log.debug("Fully authenticated " + AuthenticationUtil.getFullyAuthenticatedUser());
        }

        if (log.isDebugEnabled()) {
            log.debug("revert nodeRef:" + nodeRef);
        }
        ...
        if (!nodeRef.getId().equals(((NodeRef) version.getVersionProperty(Version2Model.PROP_FROZEN_NODE_REF)).getId())) {
        ...
        try {
            ...
            for (QName prop : propsToLeaveAlone) {
                if (oldProps.containsKey(prop)) {
                    ...
                    log.debug("keeping property:" + prop.getLocalName() + " " + oldProps.get(prop));
                }
            }
                ...
                if (!children.contains(versionedChild)) {
                    ...
                    if (this.nodeService.exists(versionedChild.getChildRef())) {
                        ...
                        if (versionedChild.isPrimary()) {
                            ...
                            if (deep && getVersionHistoryNodeRef(versionedChild.getChildRef()) != null) {
            ...
            for (AssociationRef versionedAssoc : this.nodeService.getTargetAssocs(versionNodeRef, RegexQNamePattern.MATCH_ALL)) {
                if (!assocsToLeaveAlone.contains(versionedAssoc.getTypeQName())) {
                    ...
                    if (this.nodeService.exists(versionedAssoc.getTargetRef())) {
