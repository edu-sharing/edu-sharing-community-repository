...
public class Edu_SharingWebDAVServlet extends HttpServlet
{
    ...
    protected WebDAVMethod createMethod(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (methodClass != null)
        {
            try
            {
                 ...
                 * edu-sharing fix
                 */
                WebDAVUser user = (WebDAVUser) request.getSession().getAttribute(BaseAuthenticationFilter.AUTHENTICATION_USER);
                method.setDetails(request, response, m_davHelper, user.getHomeNode());
    ...
    public void init(ServletConfig config) throws ServletException
    {
      ...
      /**  WebApplicationContext wc = WebApplicationContextUtils.getRequiredWebApplicationContext(getServletContext());
        ...
        /**
         * edu-sharing fix dont use bean cause bean would try to access webdav classes in edu-sharing webapp from alfresco context -> LinkerProblem
         */
        m_davHelper = new WebDAVHelper();
        //m_davHelper = (WebDAVHelper) context.getBean("webDAVHelper");
        m_davHelper.setServiceRegistry(serviceRegistry);
        m_davHelper.setNodeService(nodeService);
        m_davHelper.setFileFolderService(serviceRegistry.getFileFolderService());
        m_davHelper.setSearchService(searchService);
        m_davHelper.setNamespaceService(namespaceService);
        m_davHelper.setDictionaryService(serviceRegistry.getDictionaryService());
        m_davHelper.setMimetypeService(serviceRegistry.getMimetypeService());
        m_davHelper.setLockService((WebDAVLockService)context.getBean("webDAVLockService"));
        m_davHelper.setActionService(serviceRegistry.getActionService());
        m_davHelper.setPermissionService(serviceRegistry.getPermissionService());
        m_davHelper.setHiddenAspect((HiddenAspect)context.getBean("hiddenAspect"));
        m_davHelper.setAuthenticationService(serviceRegistry.getAuthenticationService());
        m_davHelper.setTenantService((TenantService)context.getBean("tenantService"));
       
      
       	Properties props = (Properties)context.getBean("global-properties");
    	m_davHelper.setSizeLimitString(props.getProperty("system.content.maximumFileSizeLimit"));
    	m_davHelper.setPolicyBehaviourFilter((BehaviourFilter)context.getBean("policyBehaviourFilter"));
       	m_davHelper.setRenameShufflePattern(Pattern.compile(props.getProperty("system.webdav.renameShufflePattern")));
       	m_davHelper.setUrlPathPrefix(props.getProperty("system.webdav.url.path.prefix"));
        m_davHelper.setEventPublisher((EventPublisher)context.getBean("eventPublisher"));
        ...
        m_davMethods.put(WebDAV.METHOD_PROPFIND, Edu_SharingPropFindMethod.class);
     ...
     */
    private void initializeRootNode(String storeValue, String rootPath, ApplicationContext context, NodeService nodeService, SearchService searchService,
