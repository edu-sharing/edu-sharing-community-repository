...
public class OnBeanCondition extends FilteringSpringBootCondition implements ConfigurationCondition {
    ...
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        if (annotations.isPresent(ConditionalOnBean.class)) {
            OnBeanCondition.Spec<ConditionalOnBean> spec = new OnBeanCondition.Spec<>(context, metadata, annotations, ConditionalOnBean.class);
        ...
        if (metadata.isAnnotated(ConditionalOnSingleCandidate.class.getName())) {
            OnBeanCondition.Spec<ConditionalOnSingleCandidate> spec = new OnBeanCondition.SingleCandidateSpec(context, metadata,
        ...
        if (metadata.isAnnotated(ConditionalOnMissingBean.class.getName())) {
            OnBeanCondition.Spec<ConditionalOnMissingBean> spec = new OnBeanCondition.Spec<>(context, metadata, annotations,
    ...
    private ConditionOutcome evaluateConditionalOnBean(OnBeanCondition.Spec<ConditionalOnBean> spec, ConditionMessage matchMessage) {
        OnBeanCondition.MatchResult matchResult = getMatchingBeans(spec);
    ...
    private ConditionOutcome evaluateConditionalOnSingleCandidate(OnBeanCondition.Spec<ConditionalOnSingleCandidate> spec,
    ...
    private ConditionOutcome evaluateConditionalOnMissingBean(OnBeanCondition.Spec<ConditionalOnMissingBean> spec,
                                                                                                               ConditionMessage matchMessage) {
        OnBeanCondition.MatchResult matchResult = getMatchingBeans(spec);
    ...
    protected final OnBeanCondition.MatchResult getMatchingBeans(OnBeanCondition.Spec<?> spec) {
        ...
        OnBeanCondition.MatchResult result = new OnBeanCondition.MatchResult();
    ...
    private ConfigurableListableBeanFactory getSearchBeanFactory(OnBeanCondition.Spec<?> spec) {
    ...
    private String createOnBeanNoMatchReason(OnBeanCondition.MatchResult matchResult) {
    ...
    private String createOnMissingBeanNoMatchReason(OnBeanCondition.MatchResult matchResult) {
        StringBuilder reason = new StringBuilder();
    ...
    private static class Spec<A extends Annotation> {
            ...
            if (types.isEmpty() && this.names.isEmpty() && this.annotations.isEmpty()) {
                try {
                    types = deducedBeanType(context, metadata);
                        ...
                        OnBeanCondition.BeanTypeDeductionException ex) {
        ...
        private Set<String> deducedBeanTypeForBeanMethod(ConditionContext context, MethodMetadata metadata) {
            try {
                ...
                throw new OnBeanCondition.BeanTypeDeductionException(metadata.getDeclaringClassName(), metadata.getMethodName(), ex);
            }
        }
    ...
    /**
     * Specialized {@link OnBeanCondition.Spec specification} for
     * {@link ConditionalOnSingleCandidate @ConditionalOnSingleCandidate}.
     */
    private static class SingleCandidateSpec extends OnBeanCondition.Spec<ConditionalOnSingleCandidate> {
