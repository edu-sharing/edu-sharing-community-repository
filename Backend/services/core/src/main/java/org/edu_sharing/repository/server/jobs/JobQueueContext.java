...
public class JobQueueContext {
    ...
    private JobQueueEntry runningJob;
     ...
     * If the current running job or the job queue mapper is not initialized, the method will exit
     ...
     * current running job and then updates this information in the database via the job queue mapper.
     *
    ...
    public void extendJobTtl() {
        if (runningJob == null || jobQueueMapper == null) {
            log.debug("no running job entry or jobqueue mapper");
            return;
        }
        log.debug("extending ttl of jobQueueEntry {}", runningJob);
        runningJob.setLastUpdated(new Date());
        jobQueueMapper.updateLastUpdated(runningJob);
    }

    /**
     * Checks whether the TTL (time-to-live) of the current running job has expired.
     * The TTL expiration is determined by adding the TTL duration to the last updated
     * timestamp of the current running job and comparing it to the current system time.
     ...
     *
     * @return true if the TTL of the current running job has expired, false otherwise
     *         or in cases where required data is not available or invalid.
     */
    public boolean isTtlExpired() {
        if (runningJob == null) {
            log.debug("no running job entry");
            return false;
        }
        ...
        Duration ttl = runningJob.getTtl();
