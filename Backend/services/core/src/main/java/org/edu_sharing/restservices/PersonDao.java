...
public class PersonDao {
	...
	public static void addNodeList(PersonDao person, String list,String nodeId) throws Exception {
		EduSharingLockHelper.runSingleton(PersonDao.class, person.getAuthorityName() + "_nodeList", () -> {
			try {
				// Simply check if node is valid
				NodeDao node = NodeDao.getNode(person.repoDao, nodeId);
				if (node.isDirectory())
					throw new IllegalArgumentException("The node " + nodeId + " is a directory. Only files are allowed for this list");
				String data = getCurrentNodeListJson(person);
				...
				updateNodeList(person, json);
			} catch(Exception e) {
				throw new RuntimeException(e);
			}
		});
	}
	private static String getCurrentNodeListJson(PersonDao person) throws Exception {
		...
		if(service.isGuest()){
			...
			data=(String) AuthorityServiceFactory.getLocalService().getAuthorityProperty(person.getUserName(), CCConstants.CCM_PROP_PERSON_NODE_LISTS);
		}
		return data;
	}
	public static List<NodeRef> getNodeList(PersonDao person, String list) throws Exception {
		String data=getCurrentNodeListJson(person);
		if(data==null)
			return null;
		...
		for(int i=0;i<array.length();i++){
			...
			try{
				...
				NodeDao.getNode(person.repoDao, nodeId);
				result.add(new NodeRef(person.repoDao.getId(), nodeId));
			}
			catch(Throwable t){
				removeNodeList(person,list,nodeId);
			}
		}
		return result;
	}

	public static void removeNodeList(PersonDao person, String list, String nodeId) throws Exception {
		EduSharingLockHelper.runSingleton(PersonDao.class, person.getAuthorityName() + "_nodeList", () -> {
			try {
				String data = getCurrentNodeListJson(person);
				if (data == null)
	...
	private static void updateNodeList(PersonDao person, JSONObject json) throws Exception {
		org.edu_sharing.service.authority.AuthorityService service=AuthorityServiceFactory.getAuthorityService(ApplicationInfoList.getHomeRepository().getAppId());
		HttpSession session = Context.getCurrentInstance().getRequest().getSession();
		if(service.isGuest()){
			...
			AuthorityServiceFactory.getLocalService().setAuthorityProperty(person.getUserName(), CCConstants.CCM_PROP_PERSON_NODE_LISTS, json.toString());
		}
	}
