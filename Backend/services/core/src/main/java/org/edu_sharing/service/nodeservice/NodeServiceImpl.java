...
public class NodeServiceImpl implements org.edu_sharing.service.nodeservice.NodeService {
    ...
    public void updateNode(String nodeId, Map<String, String[]> props, boolean obeyMds) throws Throwable {
        ...
        Map<String, Object> toSafeProps = getToSafeProps(props, obeyMds, nodeType, aspects, nodeId, parentId, null);
    ...
    public String createNode(String parentId, String nodeType, Map<String, String[]> props) throws Throwable {
        Map<String, Object> toSafeProps = getToSafeProps(props, true, nodeType, null, null, parentId, null);
    ...
    private Map<String, Object> validatePropertiesByDefinition(Map<String, String[]> props) {
        ...
        for (Map.Entry<String, String[]> e : props.entrySet()) {
            PropertyDefinition propertyDefinition = dictionaryService.getProperty(QName.createQName(e.getKey()));
            if (propertyDefinition == null) {
                throw new IllegalArgumentException("property " + e.getKey() + " is not defined in content model");
            }
            NodeServiceHelper.convertMutlivaluePropToGeneric(e.getValue(), converted, e.getKey());
        }
        return converted;
    }
    ...
    private List<ChildAssociationRef> getChildrenAssocsByType(StoreRef store, String nodeId, String type) {
        return this.getChildAssocs(new NodeRef(store, nodeId), Collections.singleton(QName.createQName(type)));
    }
    ...
    protected String getMLTextString(Object _mlText) {
        ...
        if (_mlText instanceof MLText) {
            ...
            if (StringUtils.isBlank(mlValueString.toString()) && !mlValueString.toString().contains(CCConstants.defaultLocale)) {
    ...
    public <T> List<T> sortNodeRefList(List<T> list, List<String> filter, SortDefinition sortDefinition) {
        ...
        list.sort((o1, o2) -> sortNodes(cache, getAsNode(o1), getAsNode(o2), sortDefinition));
        return list;
    }
    ...
    public List<ChildAssociationRef> getChildrenChildAssociationRefType(String parentID, String childType) {
        ...
        if (childType == null) {
            ...
            return this.getChildAssocs(parentNodeRef, new HashSet<>(List.of(QName.createQName(childType))));
        }
    ...
    public void revokeNode(String storeProtocol, String storeId, @NodePermission(CCConstants.PERMISSION_COORDINATOR) String nodeId, RevokeDetails details) throws Throwable {
        ...
        serviceRegistry.getRetryingTransactionHelper().doInTransaction(() -> {
            ...
            if(details.isUnpublish()) {
                this.serviceRegistry.getPermissionService().clearPermission(ref, CCConstants.AUTHORITY_GROUP_EVERYONE);
            }
    ...
    private void cleanupAllCollections(NodeRef ref) {
        new UsageDao(RepositoryDao.getRepository(RepositoryDao.HOME)).getUsagesByNodeCollection(ref.getId()).forEach(
                u -> CollectionServiceFactory.getLocalService().removeFromCollection(u.getCollection().getRef().getId(), u.getResourceId())
        );
    }
    ...
    public void createHandle(NodeRef nodeRef, List<String> publishedCopies, HandleService handleService, HandleMode handleMode) throws Exception {
        ...
        if (handleMode.equals(HandleMode.update) && !publishedCopies.isEmpty()) {
    ...
    public void setTemplateProperties(String nodeId, Map<String, String[]> props) throws Throwable {
        ...
        Map<String, Object> toSafeProps = getToSafeProps(props, true, nodeType, null, template, nodeId, "io_template");
    ...
    private Serializable validatePropertyByDefinition(QName prop, Serializable value) {
        PropertyDefinition propertyDefinition = dictionaryService.getProperty(prop);
        if (propertyDefinition == null) {
            throw new IllegalArgumentException("property " + prop + " is not defined in content model");
        }
