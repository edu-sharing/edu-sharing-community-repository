...
public class GlobalShareServiceImpl implements GlobalShareService {
    ...
    public String createShare(String nodeId, String[] emails, long expiryDate, String password, String emailMessageLocale) throws EMailValidationException, EMailSendFailedException, ExpiryDateValidationException,
        ...
        if (!toolPermissionService.hasToolPermission(CCConstants.CCM_VALUE_TOOLPERMISSION_INVITE_LINK)) {
        ...
        NodeRef personNodeRef = personService.getPerson(authService.getCurrentUserName());
        Map<QName, Serializable> personProps = nodeService.getProperties(personNodeRef);
        String nameInvitor = personProps.get(QName.createQName(CCConstants.CM_PROP_PERSON_FIRSTNAME)) + " " + personProps.get(QName.createQName(CCConstants.CM_PROP_PERSON_LASTNAME));
        ...
        for (String email : emails) {
            ...
            shareInfoService.createShare(io.getId(), authService.getCurrentUserName(), createdChild.getChildRef().getId(), ShareType.LINK);
            ...
            if (!EMAIL_TYPE_LINK.equals(email)) {
                ...
                try {
                    ...
                    shareInfoService.removeShare(io.getId(), authService.getCurrentUserName(), createdChild.getChildRef().getId());
    ...
    private void throwIfScopedNode(NodeRef io) {
        if (nodeService.getProperty(io, QName.createQName(CCConstants.CCM_PROP_SCOPE_TYPE)) != null) {
    ...
    public void removeShare(String nodeId, String shareNodeId) {
        NodeRef nodeRef = new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, shareNodeId);
        String owner = ownableService.getOwner(nodeRef);
        nodeService.deleteNode(nodeRef);
        shareInfoService.removeShare(nodeId, owner, shareNodeId);
    }
    ...
    public void updateDownloadCount(Share share) {
        ...
        nodeService.setProperty(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, share.getNodeId()), SHARE_PROP_DOWNLOAD_COUNTER, share.getDownloadCount());
    }
    ...
    public void updateShare(Share share) {
        ...
        if (share.getPassword() == null) {
            props.put(SHARE_PROP_PASSWORD, nodeService.getProperty(shareNodeRef, SHARE_PROP_PASSWORD));
        ...
        throwIfScopedNode(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, share.getIoNodeId()));
        nodeService.setProperties(shareNodeRef, props);
    }
    ...
    public Share getShare(String nodeId, String token) {
        List<ChildAssociationRef> childAssocRefs = nodeService.getChildAssocsByPropertyValue(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, nodeId), SHARE_PROP_SHARE_TOKEN, token);
        if (childAssocRefs.size() == 1) {
            NodeRef shareNodeRef = childAssocRefs.get(0).getChildRef();
            return getNodeShareObject(nodeId, shareNodeRef);

        } else {
            log.error("invalid number of share objects for io:{} {}", childAssocRefs.size(), nodeId);
            return null;
        }
    }
    ...
    public boolean isNodeAccessibleViaShare(NodeRef sharedNode, String accessNodeId) {
        ...
        for (ChildAssociationRef ref : nodeService.getChildAssocs(sharedNode)) {
