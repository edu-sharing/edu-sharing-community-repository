...
<#macro mv_40e_fix string>${string?replace("\n", "__")}</#macro>
...
<#macro nodeMetaDataJSON nodeMetaData filter>
      {
         ...
         , "properties": {
           <#list nodeMetaData.properties?keys as propName>
                ...
                <#--"${propName}": <@mv_40e_fix "${nodeMetaData.properties[propName]}"/> <#if propName_has_next>,</#if>-->
                "${propName}": <#if propName == "sys:archivedOriginalParentAssoc"> <@mv_40e_fix "${nodeMetaData.properties[propName]}"/> <#else>${nodeMetaData.properties[propName]}</#if> <#if propName_has_next>,</#if>
           </#list>
         }
         </#if>
         </#if>
