...
public class MCAlfrescoAPIClient extends MCAlfrescoBaseClient {
	 ...
	 * @throws Exception
	 */
	public HashMap<String, Object> getPropertiesCached(NodeRef nodeRef, boolean getFromCache, boolean checkModified, boolean ifNotInCacheReturnNull)
			throws Exception {
		...
		if (propMap == null){
			propMap = nodeService.getProperties(nodeRef);
		}
					...
					while (iter.hasNext()) {
						...
						if (AuthorityType.getAuthorityType(alfAuthority).equals(AuthorityType.GROUP)) {
							...
							if(groupNodeRef == null) {
								logger.warn("no group object found for:" + alfAuthority);
								continue;
							}
