...
-   The application contains older components build with old Angular versions, when building new features however, always look for the best current Angular pattern and suggest refactoring if it seems necessary
-   use custom `esIcon` directive instead of `mat-icon`
-   Prefer signals and computed if feasible and make use of Angular >= 21 features when it makes sense for the task
-   Prefer signal view queries (`viewChild()` / `viewChildren()` + `computed()`) over the legacy `@ViewChild` setter-with-`setTimeout` workaround for "ExpressionChangedAfterItHasBeenChecked" — signal queries settle cleanly and can be combined/filtered in a `computed()` that templates read directly (Angular 21). A `viewChild()` signal is itself a callable `() => T | undefined`, so it can be passed straight into a child `@Input()` (type it `Signal<T | undefined>`, not `() => T`) and the child reads it reactively as `this.x?.()` — handy for handing a parent-owned, possibly-`@if`-gated element down to a child without snapshot/timing bugs.
-   **Template reference variables shadow component members of the same name in template expressions.** A `#foo` ref and a component property `foo` collide: `[input]="foo"` resolves the RHS to the _ref_ (the element/component instance), not the property — e.g. binding a signal property fails with "Type Xxx is not assignable to type Signal<Xxx>". Give the `#ref` a distinct name (e.g. `#fooRef`) and point the `viewChild('fooRef')` locator at it.
...
-   Node-list click outputs (`clickItem` / `dblClickItem` on `es-node-entries-wrapper`) emit a **`NodeClickEvent<Node>`** (`{ element, source, ctrlKey?, attribute? }`), and `NodeHelperService.navigateToNode(event)` consumes the whole event (uses `ctrlKey`/`source`). Forward the full `$event`, **not** `$event.element`, when re-emitting to such a handler. `NodeClickEvent` and `ClickSource` are exported from `ngx-edu-sharing-ui` via the `index.ts` wildcard (`export * from './node-entries/entries-model'`), even though they are not listed by name.
...
-   **Rounded container card**: `roundedContainerCard()` (frame — margin / `--roundedContainerBackground` / `--borderRadiusCards` / shadow + mobile bottom-nav `margin-bottom` clearance) and `roundedContainerStaticScroll()` (makes it a static card that scrolls its content internally: `display:flex; flex-direction:column; overflow:hidden` + `height: calc(100vh - 2×--roundedContainerMargin - --mainnavCurrentHeight)` with the `$mobileTabNavHeight` subtraction). Pair with a scrolling child `flex:1; min-height:0; overflow-y:auto`. Used by collections-, editorial- and search-page `.main-content`.
-   **Pinning a sub-header above the scroll inside such a card**: because the card scrolls internally, a `position:sticky` header still sits _inside_ the scrollport (the scrollbar runs its full height). To make the scrollbar start _below_ a fixed header, move the scroll boundary inward — make the bounded ancestor `overflow:hidden; display:flex; flex-direction:column`, give the header `flex-shrink:0`, and put the scroll on an inner `flex:1; min-height:0; overflow-y:auto` wrapper.
-   **Scrollable grid/flex item collapse gotcha**: a grid item (or flex child) that is itself a scroll container (`overflow:auto/hidden`) with `min-height:0` has an automatic minimum size of `0`, so in an `auto`-sized track it collapses to zero height and disappears. Only safe when the track has a definite size (e.g. `grid-template-rows:1fr` in a height-bounded container); for `auto` rows set `overflow:visible; min-height:auto` instead.
-   **Page shell pattern** (no body scrollbar): a page `:host` is a viewport-locked flex column — `height/min-height: calc(100vh - var(--mainnavCurrentHeight)); overflow: hidden` — with the card (`.main-content`) scrolling internally. Always use `--mainnavCurrentHeight` (not `--mainnavHeight`) for layout heights; it is defined as `var(--mainnavHeight)` in `variables-root.scss` and is what the card mixins use. The main nav (`es-main-nav`) is `position: sticky` and in-flow. Gotcha: an opacity-only `[@fadeOut]` `mat-progress-bar` still occupies its height in flow — `position: absolute` it so it doesn't push the page past the viewport.
-   **`roundedContainerCard` open-drawer nudge**: it shrinks the card's margin (to 4px) on the side of an open `mat-sidenav`, but only if the host page **manually binds** `[class.mat-sidenav-container-left-open]`/`[class.mat-sidenav-container-right-open]` on its own `<mat-sidenav-container>` (these are app classes, not Angular Material defaults), and the mixin is included on a **top-level** `.main-content` selector (never nested under `mat-sidenav-container { … }`, or its `mat-sidenav-container.…-open &` selector gains a second non-matching ancestor).
-   Shared static SCSS vars live in `variables-scss.scss` (e.g. `$nodeEntriesTableRowHeight`, `$mobileSidebarModal`, `$mobileTabSwitchWidth`/`$mobileTabNavHeight`). Add/reuse one there rather than hardcoding a value duplicated across component stylesheets.
