...
import {
    ...
    User,
    UserSimple, Person, Node, GenericAuthority, ListItemSort
...
export class PermissionsAuthoritiesComponent implements OnChanges, AfterViewInit {
    ...
    private getMemberOptions(): OptionItem[] {
        ...
        if(this.editMembers || this.editGroups) {
            const removeMembership = new OptionItem('PERMISSIONS.MENU_REMOVE_MEMBERSHIP', 'delete', (data) =>
                this.deleteMembership(NodeHelperService.getActionbarNodes(this.nodeMemberAdd.getSelection().selected, data))
            );
            ...
            const removeFromGroup = new OptionItem('PERMISSIONS.MENU_REMOVE_MEMBER', 'delete', (data) =>{
                ...
                this.deleteMember(NodeHelperService.getActionbarNodes(this.nodeMemberAdd.getSelection().selected, data))
            });
    ...
    ngAfterViewInit(): void {
        this.nodeEntries.getSelection().changed.subscribe((selection) =>
            this.onSelection.emit(selection.source.selected)
        );
    }
  ...
  private getList<T>(data: T): T[] {
    return NodeHelperService.getActionbarNodes(this.nodeEntries.getSelection().selected as unknown as T[], data);
  }
  private async updateOptions() {
      if (this.embedded) {
          ...
          const orgSignupList = new OptionItem('PERMISSIONS.ORG_SIGNUP_LIST', 'playlist_add', async (data) => {
              ...
              this.groupSignupList.setData(await this.iam.getGroupSignupList(this.groupSignup.authorityName).toPromise());
          ...
          const signupAdd = new OptionItem('PERMISSIONS.ORG_SIGNUP_ADD', 'person_add', (node: UserSimple) => {
              this.toast.showProgressDialog();
              const users = NodeHelperService.getActionbarNodes(this.signupList.getSelection().selected, node);
  ...
  public loadAuthorities(event: FetchEvent = null) {
    ...
    const request = {sortBy: [sort], sortAscending: this.sortConfig.direction === 'asc', offset: this.dataSource.getData().length};
    ...
    if (this._mode === 'ORG') {
      ...
      this.organization.getOrganizations(query, !this.isAdmin, request).subscribe(async (orgs: OrganizationOrganizations) => {
          await this.dataSource.appendData(orgs.organizations.filter(o => o.administrationAccess));
      ...
      if (this.org) {
        this.iam.getGroupMembers(this.org.authorityName, query, this._mode, request).subscribe(async (data: IamAuthorities) => {
        ...
        this.iam.searchGroups(query, true, '', '', request).subscribe(async (data: IamGroups) => {
            this.dataSource.setPagination(data.pagination);
  ...
  refreshMemberList() {
    ...
    if (this.addMembers){
      if (this.org && this.addMembers.authorityName != this.org.authorityName){
        ...
        this.iam.getGroupMembers(this.org.authorityName, this.manageMemberSearch, RestConstants.AUTHORITY_TYPE_USER, request).subscribe(async (data: IamAuthorities) => {
        ...
        this.iam.searchUsers(this.manageMemberSearch, true, '', request).subscribe(async (data) => {
            this.memberList.setPagination(data.pagination);
        ...
        this.iam.getUserGroups(this.editGroups.authorityName, this.manageMemberSearch, request).subscribe(async (data) => {
            this.memberList.setPagination(data.pagination);
      ...
      this.iam.getGroupMembers((this.editMembers as Group).authorityName, this.manageMemberSearch, RestConstants.AUTHORITY_TYPE_USER, request).subscribe(async (data) => {
          this.memberList.setPagination(data.pagination);
          await this.memberList.appendData(data.authorities);
          this.memberList.isLoading = false;
      });
  ...
  saveGroupSignup() {
    ...
    onClick(event: NodeClickEvent<GenericAuthority>) {
        if(this._mode === 'ORG') {
            if(!this.nodeEntries.getSelection().isEmpty() &&
                event.element === this._org
            ) {
                this._org = null;
    ...
    selectOnClick(source: NodeEntriesWrapperComponent<GenericAuthority>, event: NodeClickEvent<GenericAuthority>) {
