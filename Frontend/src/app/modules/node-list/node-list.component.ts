...
export class NodeListComponent {
    ...
    @Input() set columns(columns: ListItem[]) {
        ...
        if (this._columns && this._columns.length) this.sortBy = this._columns[0].name;
    }
    @Input() options: OptionItem[];
    @Input() sortBy: string;
    @Input() sortAscending = true;
    @Input() set reload(reload: Boolean) {
        if (reload) this.doReload();
    }
    ...
    constructor(private connector: RestConnectorService, private toast: Toast) {
