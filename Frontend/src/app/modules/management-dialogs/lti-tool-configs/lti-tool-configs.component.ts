...
export class WorkspaceLtiToolConfigsComponent {
    ...
    public constructor(
        ...
        this.actionOptions.push(
            new OptionItem('WORKSPACE.LTI_TOOLS.NEW_CONFIG', 'add', () => {
                this.onClose.emit();
                this.onCreate.emit({ type: MdsType.ToolInstance, node: this._tool });
            }),
        );
        this.explorerOptions.addOptions.push(
            new OptionItem('WORKSPACE.LTI_TOOLS.EDIT_CONFIG', 'edit', (node: Node) => {
                this.onEdit.emit(node);
                this.onClose.emit();
            }),
        );
        this.explorerOptions.addOptions.push(
            new OptionItem('WORKSPACE.LTI_TOOLS.DELETE_CONFIG', 'delete', (node: Node) =>
