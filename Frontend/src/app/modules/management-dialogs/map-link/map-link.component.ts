...
export class MapLinkComponent  {
  ...
  private createLink() {
    ...
    properties[RestConstants.CCM_PROP_MAP_REF_TARGET] = [RestHelper.createSpacesStoreRef(this._node)];
    this.toast.showProgressDialog();
      this.nodeApi.createNode(this.breadcrumbs[this.breadcrumbs.length-1].ref.id,
          RestConstants.CCM_TYPE_MAP,
          [RestConstants.CCM_ASPECT_MAP_REF],
          properties).subscribe(({node}) => {
        const additional= {
          link:{
            caption:'MAP_LINK.CREATED_LINK',
            callback:()=> {
              UIHelper.goToWorkspaceFolder(this.nodeApi,this.router,this.connector.getCurrentLogin(),
                  this.breadcrumbs[this.breadcrumbs.length-1].ref.id);
            }
          }
        };
        this.toast.toast('MAP_LINK.CREATED', {folder: this.breadcrumbs[this.breadcrumbs.length - 1].name}, null, null, additional);
