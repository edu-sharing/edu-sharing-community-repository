...
export class WorkspaceLtiToolsComponent {
        ...
        if (node) {
            ...
            this.toolService.getToolInstances(node.ref.id).subscribe((data: NodeList) => {
                ...
                if (node.access.indexOf(RestConstants.ACCESS_WRITE) != -1) {
                    this.options.push(
                        new OptionItem('WORKSPACE.LTI_TOOLS.EDIT_TOOL', 'edit', () =>
                            this.onEdit.emit(node),
                        ),
                    );
                }
                this.options.push(
                    new OptionItem('WORKSPACE.LTI_TOOLS.MANAGE_CONFIGS', 'settings', () =>
                        this.onOpenConfig.emit(node),
                    ),
                );
            });
        } else {
            this.options.push(
                new OptionItem('WORKSPACE.LTI_TOOLS.NEW_TOOL', 'add', () =>
                    this.onCreate.emit({ type: MdsType.ToolDefinition }),
                ),
            );
        }
    }
    ...
    public search() {
        this.loading = true;
        let criterias = [{ property: 'toolname', values: [this.query] }];
        this.searchService
            .search(
                criterias,
                [],
    ...
    doRefresh() {
        ...
        if (this._currentTool) {
            this.nodeService
                .getNodeMetadata(this._currentTool.ref.id, [RestConstants.ALL])
