...
export class WorkspaceContributorComponent {
    ...
    public addVCard(mode = this.editMode) {
        ...
        this.editScopeNew =
            this.editMode == 'lifecycle' ? this.rolesLifecycle[0] : this.rolesMetadata[0];
    }
    openDatepicker() {
        this.date = new Date();
    ...
    public saveEdits() {
        ...
        this.edit.contributeDate = this.date
            ? DateHelper.getDateFromDatepicker(this.date).toISOString()
            : null;
        let array =
            this.editMode == 'lifecycle' ? this.contributorLifecycle : this.contributorMetadata;
