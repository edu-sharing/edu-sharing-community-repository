...
export class WorkspaceFileUploadComponent {
    ...
    private upload(number: number) {
        if (number >= this._files.length) {
            if (this.error) this.showClose = true;
            else this.onDone.emit(this.resultList);
            return;
        }
        ...
        this.node
            .createNode(
                this.current ? this.current.ref.id : RestConstants.INBOX,
                RestConstants.CCM_TYPE_IO,
                [],
                RestHelper.createNameProperty(this._files.item(number).name),
                true,
            )
            .subscribe(
                (data: NodeWrapper) => {
