...
@Component({
    selector: 'app-workspace-metadata-block',
...
export class WorkspaceMetadataBlockComponent {
    ...
    private format(node: Node): any {
        ...
        data.creator = ConfigurationHelper.getPersonWithConfigDisplayName(node.createdBy, this.configService);
    ...
    private formatPermissions(permissions: NodePermissions): any {
        ...
        for (const permission of permissions.permissions.inheritedPermissions) {
            if (permission.authority.authorityName === currentAuth || permission.authority.authorityType === RestConstants.AUTHORITY_TYPE_OWNER) {
