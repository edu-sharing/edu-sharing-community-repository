...
export class AutocompleteComponent {
    ...
    itemChosen(item: SuggestItem) {
        if (typeof this.chosen != 'undefined') {
            for (var i = 0; i < this.chosen.length; i++) {
                if (this.chosen[i].title == item.title) return true;
            }
        }
        return false;
    }
    ...
    handleKeyDown(event: any) {
        if (
            ...
            if (event.keyCode == 40) {
                this.activeItem++;
                if (this.activeItem >= this._suggestions.length) this.activeItem = 0;
            }

            if (event.keyCode == 38) {
                this.activeItem--;
                if (this.activeItem < 0) this.activeItem = this._suggestions.length - 1;
            }
...
export class SuggestItem {
    ...
    constructor(
        ...
        public materialIcon: string = null,
